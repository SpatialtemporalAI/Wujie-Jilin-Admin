# Graph Report - Wujie-Jilin-Admin  (2026-09-01)

## Corpus Check
- 1055 files · ~1,222,716 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 11536 nodes · 22131 edges · 1612 communities (909 shown, 703 thin omitted)
- Extraction: 66% EXTRACTED · 34% INFERRED · 0% AMBIGUOUS · INFERRED: 7440 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9a1001fa`
...
1. `NotFoundError` - 334 edges
2. `ResponseModel` - 323 edges
3. `PageRequest` - 321 edges
4. `BaseRespEntity` - 221 edges
5. `ConflictError` - 220 edges
6. `Robot` - 193 edges
7. `BaseEntity` - 184 edges
8. `ResponsePageModel` - 177 edges
9. `BaseReqEntity` - 145 edges
10. `ForbiddenError` - 133 edges
...
## Communities (1612 total, 703 thin omitted)
...
Cohesion: 0.11
Nodes (46): ORJSONResponse, Request, str, Request, str, Exception, FastAPI, ORJSONResponse (+38 more)

### Community 2 - "Community 2"
Cohesion: 0.07
Nodes (57): Redis, Any, bool, float, int, Redis, str, Redis (+49 more)

### Community 3 - "Community 3"
Cohesion: 0.20
Nodes (53): AsyncSession, int, PageRequest, Request, str, SysConfigBatchUpdate, SysConfigCreate, SysConfigQueryParams (+45 more)

### Community 4 - "Community 4"
Cohesion: 0.13
Nodes (50): Any, AsyncSession, bool, int, PageRequest, RobotFaceRecognitionCreate, RobotFaceRecognitionUpdate, RobotVoiceConfigSchema (+42 more)

### Community 5 - "Community 5"
Cohesion: 0.08
Nodes (72): AsyncSession, bool, EditorSaveRequest, int, SysUser, UploadFile, AsyncSession, int (+64 more)
...
Cohesion: 0.03
Nodes (60): AllRole, ChangePasswordRequest, CommonSearchParams, Config, ConfigBatchUpdate, ConfigCreate, ConfigGroup, ConfigList (+52 more)
...
Cohesion: 0.11
Nodes (98): AsyncSession, int, PageRequest, Request, str, SysDictBatchUpdateStatus, SysDictCreate, SysDictItemBatchUpdateStatus (+90 more)

### Community 10 - "Community 10"
Cohesion: 0.11
Nodes (37): BaseSettings, str, BaseModel, get_current_env(), get_env_file_path(), import_config_class(), load_config(), 加载多环境配置的核心入口：     1. 获取当前环境标识     2. 动态导入对应的配置类     3. 配置 .env 文件路径（若存在） (+29 more)

### Community 11 - "Community 11"
Cohesion: 0.10
Nodes (33): 创建一个权限校验依赖项      Args:         permission_code: 权限标识码，如 "sys:menu:add"      Retu, 创建一个权限校验依赖项      Args:         permission_code: 权限标识码，如 "sys:menu:add", 创建一个权限校验依赖项，只要拥有任意一个权限码即通过（OR 关系）。      用于同一接口被多个页面共享、且各自有独立权限码的场景，     例如 sc, require_any_permission(), require_permission(), str, AsyncSession, str (+25 more)

### Community 13 - "Community 13"
Cohesion: 0.10
Nodes (24): Any, bool, bytes, int, str, main(), MerchantOpenApiTester, openapi_call() (+16 more)
...
Cohesion: 0.17
Nodes (26): AsyncSession, int, PageRequest, SysUser, VoiceConsultationSessionQueryParams, AsyncSession, datetime, int (+18 more)
...
Cohesion: 0.09
Nodes (42): bool, int, ORJSONResponse, Request, str, bool, int, Request (+34 more)
...
Cohesion: 0.22
Nodes (20): AsyncSession, int, PageRequest, Request, SysUser, SysUserCreate, SysUserPasswordUpdate, SysUserQueryParams (+12 more)

### Community 20 - "Community 20"
Cohesion: 0.04
Nodes (96): str, str, str, int, str, BaseEntity, BaseRespEntity, CallLogDetailResponse (+88 more)

### Community 21 - "Community 21"
Cohesion: 0.05
Nodes (81): ABC, int, str, WebSocket, Any, int, str, lifespan() (+73 more)
...
Cohesion: 0.06
Nodes (84): bool, int, str, AsyncSession, bool, datetime, int, str (+76 more)

### Community 24 - "Community 24"
Cohesion: 0.07
Nodes (35): _is_multi_tenant_enabled(), 获取当前认证的用户（同一请求内缓存，避免重复查询）, 自动选择租户：优先使用上次登录的租户，否则使用第一个租户。         Returns: (tenant_id, tenant_list, secret_, 自动选择租户：优先使用上次登录的租户，否则使用第一个租户。         Returns: (tenant_id, tenant_list, secret_, 验证token中的session_id是否有效         Args:             token: JWT令牌         Return, 获取当前认证的用户（同一请求内缓存，避免重复查询）, 获取当前认证的用户（同一请求内缓存，避免重复查询）, 获取用户信息，包含角色列表         Args:             user_id: 用户ID         Returns: (+27 more)

### Community 25 - "Community 25"
Cohesion: 0.08
Nodes (30): async_sessionmaker, AsyncEngine, Any, AsyncSession, bool, DatabaseModel, str, AsyncDatabaseManager (+22 more)
...
Cohesion: 0.08
Nodes (25): AliyunSMS, 发送app通知         Args:             alert_data: 报警数据, 验证用户凭据         通过手机号查找用户并验证验证码         Args:             phone: 手机号, 刷新access token         Args:             refresh_token: 刷新令牌         Returns:, 验证token中的session_id是否有效         Args:             token: JWT令牌         Return, 用户注册后的回调         可以在这里实现用户注册后的额外逻辑，如发送欢迎邮件等         Args:             user: 注, 用户注册后的回调         可以在这里实现用户注册后的额外逻辑，如发送欢迎邮件等         Args:             user: 注, 获取当前认证的用户         这是一个直接可用的FastAPI依赖项，封装了JWTAuthManager.current_user方法， (+17 more)

### Community 28 - "Community 28"
Cohesion: 0.09
Nodes (20): createRouterGuard(), createProgressGuard(), createRouteGuard(), createDocumentTitleGuard(), setDayjsLocale(), getLocale(), setupI18n(), setupAppVersionNotification() (+12 more)

### Community 30 - "Community 30"
Cohesion: 0.09
Nodes (45): Any, bytes, str, AsyncSession, ExportTaskSubmit, int, str, SysUser (+37 more)

### Community 31 - "Community 31"
Cohesion: 0.15
Nodes (23): AsyncSession, bool, FastAPI, PluginBase, str, PluginBase, 插件安装时调用（仅执行一次）。         在 alembic 迁移完成后执行，用于种子数据写入、菜单初始化等。, 插件卸载时调用（仅执行一次）。         在 alembic 生成删除迁移前执行，用于清理种子数据。 (+15 more)

### Community 32 - "Community 32"
Cohesion: 0.06
Nodes (34): devDependencies, consola, @elegant-router/vue, eslint, eslint-plugin-vue, @iconify/json, kolorist, mammoth (+26 more)
...
Cohesion: 0.06
Nodes (45): 1. 概述, 2.1 凭证, 2.2 请求头, 2.3 签名算法, 2.4 关键注意事项, 2.5 签名示例（Python）, 2.6 签名示例（Shell / curl）, 2. 鉴权机制 (+37 more)
...
Cohesion: 0.39
Nodes (13): AsyncSession, int, LoginLogQueryParams, PageRequest, SysUser, batch_delete_logs(), clear_logs(), delete_log() (+5 more)

### Community 40 - "Community 40"
Cohesion: 0.06
Nodes (32): angleDeg, ann, baseContextMenuOptions, confirmRename(), contextMenuOptions, cursorX, handleContextMenu(), handleCursorPosition() (+24 more)

### Community 41 - "Community 41"
Cohesion: 0.31
Nodes (41): AsyncSession, int, PageRequest, Request, RobotFaceRecognitionCreate, RobotFaceRecognitionUpdate, RobotVoiceConfigSchema, SysUser (+33 more)

### Community 42 - "Community 42"
Cohesion: 0.05
Nodes (44): Any, RobotEventLogDetailResponse, RobotEventLogQueryParams, RobotEventLogResponse, GrpcServiceConfig, 机器人型号查询参数模型     用于型号列表分页查询时的筛选条件, 机器人型号创建请求模型     用于创建新型号时的请求数据, 机器人型号更新请求模型     用于更新型号信息时的请求数据 (+36 more)

### Community 43 - "Community 43"
Cohesion: 0.12
Nodes (43): Any, AsyncSession, bool, float, int, RetryCallResult, str, timedelta (+35 more)

### Community 44 - "Community 44"
Cohesion: 0.18
Nodes (14): Any, bool, int, Redis, str, Any, bool, int (+6 more)

### Community 45 - "Community 45"
Cohesion: 0.26
Nodes (20): AutoMcpToolCreate, McpToolTestRequest, AutoMcpToolCreate, McpToolTestRequest, str, McpServerStatusResponse, McpToolInfo, AutoMcpToolCreate (+12 more)
...
Cohesion: 0.08
Nodes (24): dependencies, @better-scroll/core, clipboard, defu, echarts, fabric, @iconify/vue, json5 (+16 more)
...
Cohesion: 0.09
Nodes (18): 系统用户密码更新请求模型     用于重置或修改用户密码, 系统用户简单响应模型     用于只需要展示基本用户信息的场景, 系统用户列表响应模型     用于用户列表展示，包含关联角色（用于编辑回显）, 系统用户列表响应模型     用于用户列表展示，不包含关联角色数据, 系统用户详细响应模型     用于展示用户完整信息，包括关联的角色, 系统用户详细响应模型     用于展示用户完整信息，包括关联的角色, 系统用户批量更新状态请求模型     用于批量启用或禁用用户, 系统用户批量更新状态请求模型     用于批量启用或禁用用户 (+10 more)
...
Cohesion: 0.05
Nodes (36): 1. 接入流程, 2.1 统一响应结构, 2.2 错误码, 2. 通用约定, 3.1 请求头, 3.2 签名算法, 3.3 防重放, 3. 鉴权机制（HMAC-SHA256） (+28 more)

### Community 53 - "Community 53"
Cohesion: 0.11
Nodes (38): Any, bool, float, int, str, BatteryThresholdChangedResponse, FaceRecognitionChangedResponse, GreetingModeChangedResponse (+30 more)

### Community 54 - "Community 54"
Cohesion: 0.31
Nodes (26): AsyncSession, int, Merchant, str, OpenApiResult, _annotation_to_point(), _assert_points_on_robot_map(), _create_nav_task() (+18 more)
...
Cohesion: 0.34
Nodes (35): AuthorizationError, Any, Exception, FastAPI, HTTPException, SysUserCreate, SysUserPasswordUpdate, SysUserUpdate (+27 more)

### Community 59 - "Community 59"
Cohesion: 0.09
Nodes (78): AsyncSession, int, PageRequest, Request, SysMenuCreate, SysMenuQueryParams, SysMenuUpdate, SysUser (+70 more)
...
Cohesion: 0.15
Nodes (20): editor.codeActionsOnSave, source.fixAll.eslint, editor.formatOnSave, typescript.tsdk, editor.codeActionsOnSave, source.fixAll.eslint, editor.formatOnSave, typescript.tsdk (+12 more)
...
Cohesion: 0.05
Nodes (41): 1. 添加到宿主项目, 2. 克隆带 submodule 的项目, 3. 拉取 submodule 的最新更新, 4. 在宿主项目里配置 Python 导入路径, 5. 在宿主项目里使用, Alembic 数据库迁移, code:bash (# 建议放在 libs/ 或 third_party/ 下，并显式指定合法的目录名), code:bash (uv sync) (+33 more)

### Community 81 - "Community 81"
Cohesion: 0.05
Nodes (48): Claude Code 接入 MCP, code:block1 (Wujie-jilin-Admin/), code:json ({), code:bash (curl http://127.0.0.1:9001/health), code:bash (cd frontend), code:bash (pnpm install), code:bash (pnpm dev), code:bash (cd backend && uv sync) (+40 more)
...
Cohesion: 0.08
Nodes (10): fetchCloseVideoMonitoring(), fetchCreateRobotModel(), fetchGetAllRobots(), fetchGetLatestRobotStatus(), fetchGetRobotList(), fetchOpenVideoMonitoring(), fetchUpdateRobotModel(), fetchVideoMonitoringHeartbeat() (+2 more)

### Community 91 - "Community 91"
Cohesion: 0.03
Nodes (17): fetchGetCaptcha(), fetchGetUserInfo(), fetchLogin(), fetchRefreshToken(), fetchVerifyCaptcha(), fetchGetApiStats(), fetchGetSystemMetrics(), fetchGetPermissions() (+9 more)
...
Cohesion: 0.06
Nodes (33): ann, any, arrowTransform, center, circle, deco, endAnn, fabricObj (+25 more)

### Community 94 - "Community 94"
Cohesion: 0.22
Nodes (4): HeaderPlugin, _headerPlugins, i18n, registerPluginI18n()
...
Cohesion: 0.20
Nodes (9): 前端, 后端, 机器人管理移除列表状态列, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)
...
Cohesion: 0.14
Nodes (13): Align, DataTableBaseColumn, DataTableExpandColumn, DataTableSelectionColumn, FormRules, SetTableColumnKey, TableColumn, TableColumnCheck (+5 more)
...
Cohesion: 0.34
Nodes (13): Redis, ConnectionPool, Redis, close_pool(), get_client(), get_pool(), get_redis_client(), get_sync_redis_client() (+5 more)

### Community 104 - "Community 104"
Cohesion: 0.05
Nodes (42): 1. 接入流程, 2.1 统一响应结构, 2.2 错误码, 2. 通用约定, 3.1 请求头, 3.2 签名算法, 3.3 防重放, 3. 鉴权机制（HMAC-SHA256） (+34 more)
...
Cohesion: 0.20
Nodes (9): 前端, 后端, 场景地图、任务列表与运行监控 UI 修复, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)
...
Cohesion: 0.11
Nodes (9): MapService, MapServiceServicer, MapServiceStub, Constructor.          Args:             channel: A grpc.Channel., 通知地图已保存：接收完整地图信息（map + labels），返回处理状态, 通知地图已保存：接收完整地图信息（map + labels），返回处理状态, 搜索所有地图：返回 id + version 列表, 搜索所有地图：返回 id + version 列表 (+1 more)
...
Cohesion: 0.12
Nodes (27): bool, bool, async_driver(), Config, ConnectionMode, DatabaseDriver, DatabaseType, DateTimeModel (+19 more)

### Community 118 - "Community 118"
Cohesion: 0.17
Nodes (7): FastAPI, str, _import_model(), MultiTenantPlugin, 在 alembic autogenerate 前调用。         导入插件模型 → 注册到 Base.metadata。         给已有模型的, 安装：创建默认租户，迁移现有数据，分配超级管理员，种子菜单。         要求 alembic 迁移已执行（表和列已存在）。, PluginBase
...
Cohesion: 0.24
Nodes (8): McpContext, str, TextContent, ToolParam, GenerateDictionary, tool_description(), tool_name(), tool_params()

### Community 122 - "Community 122"
Cohesion: 0.13
Nodes (23): bool, int, bool, int, str, build_database_url(), DatabaseCredentials, DatabaseType (+15 more)
...
Cohesion: 0.08
Nodes (23): code:bash (cd backend), code:bash (cd frontend), code:bash (cd backend), drop 迁移 downgrade 完整保留字段, ORM 模型不 import schemas 模块, task_execution_record 表的 Pydantic 类早已有, 不引入运行时校验，只补文档, 关键决策 (+15 more)
...
Cohesion: 0.19
Nodes (13): header, sider, tab, base-text, container, inverted, layout, colors (+5 more)
...
Cohesion: 0.29
Nodes (21): AsyncSession, bool, int, Select, str, SysUser, SysUserQueryParams, _apply_user_filters() (+13 more)

### Community 137 - "Community 137"
Cohesion: 0.15
Nodes (12): code:text (.), code:powershell (uv run python .\main.py), code:python (PROTO_BASE_DIR = Path("./protos").resolve()), code:text (protos/map/*.proto), code:text (generated/map/), code:toml (dependencies = [), code:powershell (# 1. 拉取主仓库和子模块), Wujie-Jilin-Grpc (+4 more)
...
Cohesion: 0.31
Nodes (8): int, str, int, str, generate_session_id(), parse_session_info(), 基于用户ID和当前时间生成会话ID     参数:         user_id: 用户唯一标识         secret_salt: 用于增强安全, 验证会话ID并解析其中的时间信息（仅用于演示，实际中哈希值无法反向解析）     注意：哈希是单向的，此函数仅用于说明生成逻辑，实际中无法从session_i

### Community 140 - "Community 140"
Cohesion: 0.16
Nodes (12): bool, int, parse_optional_enum(), parse_optional_int(), parse_positive_int(), 生成可选枚举校验器：空值收敛为 None，合法值原样返回，非法值抛错。      与 OptionalIntField 同款 BeforeValidator, 生成可选枚举校验器：空值收敛为 None，合法值原样返回，非法值抛错。      与 OptionalIntField 同款 BeforeValidator, 生成分页整数的 BeforeValidator：空值/非法值收敛为 default，<1 取 1，超过 max_value 截断。      用于 Page (+4 more)
...
Cohesion: 0.15
Nodes (11): code:block1 (修复前：SELECT count(*) AS count_1            ← 无 FROM，PG 返回 1), 为什么只有部分页面中招, 修复分页 count 恒返回 1（操作日志等纯实体查询页）, 关键决策, 根因（关键）, 涉及范围, 状态, 相关文件 (+3 more)
...
Cohesion: 0.06
Nodes (104): MapInfo, SceneMap, str, int, RetryCallResult, AsyncSession, int, PageRequest (+96 more)
...
Cohesion: 0.09
Nodes (22): 不引入 enum 列, 保留前端图标类型单选交互, 关键决策, 前端, 后端, 场景 1：本地图标保存生效, 场景 2：iconify 图标不受影响, 场景 3：旧数据迁移 (+14 more)

### Community 150 - "Community 150"
Cohesion: 0.16
Nodes (38): 获取用户信息         Args:             user_id: 用户ID         Returns:, 获取用户信息         Args:             user_id: 用户ID         Returns:, 更新用户信息（PostgreSQL + SQLAlchemy异步版）, 更新用户信息（PostgreSQL + SQLAlchemy异步版）, update_user_info(), AsyncSession, int, Request (+30 more)
...
Cohesion: 0.29
Nodes (6): [], allPages, component, { layout, page }, params, prefix
...
Cohesion: 0.13
Nodes (15): Run migrations in 'online' mode.      In this scenario we need to create an En, Run migrations in 'online' mode.      In this scenario we need to create an En, Run migrations in 'online' mode.      In this scenario we need to create an En, Run migrations in 'online' mode.      In this scenario we need to create an En, Run migrations in 'offline' mode.      This configures the context with just a U, Run migrations in 'offline' mode.      This configures the context with just a, Run migrations in 'offline' mode.      This configures the context with just a, Run migrations in 'online' mode.      In this scenario we need to create an Engi (+7 more)

### Community 160 - "Community 160"
Cohesion: 0.18
Nodes (19): bool, bytes, float, int, Request, Response, str, MerchantCallLog (+11 more)
...
Cohesion: 0.06
Nodes (30): 1. 启动机器人（默认会启动配网模式）, 1. 配置电脑的以太网, 2. 手机连接机器人热点, 2、连接网线, 3. SSH 登录机器人, 3. 打开配网页面, 3. 确认网络连通, 4. 填写 WiFi 信息 (+22 more)
...
Cohesion: 0.11
Nodes (37): AsyncSession, int, PageRequest, Request, str, SysUser, TaskExecutionRecordQueryParams, _build_start_msg() (+29 more)

### Community 178 - "Community 178"
Cohesion: 0.17
Nodes (25): int, str, Any, CustomErrorCode, str, CustomResponse, CustomResponseCode, Any (+17 more)
...
Cohesion: 0.18
Nodes (10): 关键决策（与用户确认）, 前端, 参数配置 · 人脸识别改为直连阿里云 facebody, 后端, 涉及范围, 状态, 相关文件, 约束与备注 (+2 more)
...
Cohesion: 0.08
Nodes (23): 1. payload 用 JSON 存储而非引用 DB 记录, 2. 测试按钮不入队的理由, 3. 指数退避只 3 次, 4. 响应契约不变（code=0）, 5. 复用现有调度框架, 6. _ROUTING 字典实现"动态调度", gRPC 推送失败持久化重试队列 + 前端"绿色 success + 备注", 关键设计决策 (+15 more)
...
Cohesion: 0.05
Nodes (31): validate(), closeDrawer(), createDefaultModel(), FormRules, handleInitModel(), handleSubmit(), isEdit, model (+23 more)

### Community 221 - "Community 221"
Cohesion: 0.40
Nodes (4): configurations, version, configurations, version
...
Cohesion: 0.10
Nodes (20): gRPC 重试任务永远 pending（retry_count 不递增）, 三种失败路径统一推进 retry_count, 不放宽 resp.success=False 路径, 不调整退避策略, 关键决策, 内层 30s 硬超时 + 外层 1600s 任务超时, 后端, 场景 1：grpc 不通（对端 IP 不可达） (+12 more)

### Community 454 - "Community 454"
Cohesion: 0.13
Nodes (8): dayjs, if(), if(), if(), if(), if(), if(), if()
...
Cohesion: 0.35
Nodes (27): AsyncSession, Merchant, Merchant, execute_task(), goto_point(), list_points(), list_robots(), list_scenes() (+19 more)

### Community 489 - "Community 489"
Cohesion: 0.10
Nodes (20): 1. 任务管理新增任务时选择地图列表 403, 2. gRPC 推送失败重试的同业务键去重, Q1, Q1：scene_map list 接口权限 OR 加入 task:list, Q1：复用 task:list，不新增独立权限, Q2, Q2： cancelled 状态而非 dead, Q2：gRPC 重试任务按业务键去重 (+12 more)
...
Cohesion: 0.06
Nodes (19): Missing associated documentation comment in .proto file., Missing associated documentation comment in .proto file., 语音配置推送服务     调用方完成 DB 写入后调用，将最新语音配置全量推送给机器人侧直接生效。     推送类 RPC（按职责拆分）：     - N, 语音配置推送服务     调用方完成 DB 写入后调用，将最新语音配置全量推送给机器人侧直接生效。     推送类 RPC（按职责拆分）：     - N, 语音配置推送服务     调用方完成 DB 写入后调用，将最新语音配置全量推送给机器人侧直接生效。     推送类 RPC（按职责拆分）：     - N, Constructor.          Args:             channel: A grpc.Channel., Constructor.          Args:             channel: A grpc.Channel., 语音配置推送服务     调用方完成 DB 写入后调用，将最新语音配置全量推送给机器人侧直接生效。     推送类 RPC（按职责拆分）：     - N (+11 more)
...
Cohesion: 0.20
Nodes (9): 前端, 后端, 接口传参类型校验收紧, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 496 - "Community 496"
Cohesion: 0.20
Nodes (9): 前端, 后端, 地图编辑器、机器人新增与任务管理修复, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)
...
Cohesion: 0.05
Nodes (37): code:json ({), code:json ({), code:block3 (前端（展示/编辑）          后端（存储/逻辑）), code:python (dt = datetime.fromisoformat(time_str)), `is_system` 字段, Status 字段桥接, 不在前端表单中填写的字段, 任务管理 · 机器人绑定契约 (+29 more)
...
Cohesion: 0.06
Nodes (28): addDialogVisible, addSceneUploading, annotations, configFileName, configUploading, contextMenuScenePoint, contextMenuShow, contextMenuX (+20 more)
...
Cohesion: 0.06
Nodes (15): [], candIdx, currentActive, data, existing, fontSize, group, hLine (+7 more)
...
Cohesion: 0.06
Nodes (30): CommonSearchParams, PointProgressStatus, ProgressDetail, Task, TaskAction, TaskActionItem, TaskActionSnapshot, TaskCreate (+22 more)
...
Cohesion: 0.08
Nodes (25): AllRobot, AllRobotModel, CommonSearchParams, GrpcServiceConfig, LocationInfo, Robot, RobotCreate, RobotGrpcConfig (+17 more)
...
Cohesion: 0.13
Nodes (18): Any, DatabaseModel, str, Any, DatabaseModel, 构建数据库连接关键字参数          Returns:             连接参数字典, 构建数据库连接关键字参数          Returns:             连接参数字典, 构建数据库连接关键字参数          Returns:             连接参数字典 (+10 more)
...
Cohesion: 0.20
Nodes (9): 删除机器人时清理任务关联, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 526 - "Community 526"
Cohesion: 0.14
Nodes (13): CommonSearchParams, CronPreviewRequest, CronPreviewResponse, RegistryTask, ScheduledTask, ScheduledTaskCreate, ScheduledTaskList, ScheduledTaskSearchParams (+5 more)
...
Cohesion: 0.04
Nodes (49): AsyncAttrs, 系统用户表     存储系统管理用户的基本信息和认证凭证, Base, DataClassBase, DateTimeMixin, LogicMixin, MappedBase, __tablename__() (+41 more)

### Community 529 - "Community 529"
Cohesion: 0.06
Nodes (58): bool, int, str, bool, bytes, int, str, StorageBackend (+50 more)
...
Cohesion: 0.10
Nodes (20): 1. 强类型 + 通用调度内核（不做"万能 dispatch"）, 2. 最终一致语义（不回滚 DB）, 3. voice 保存的智能拆分, 4. 地址 Provider 抽象（为数据库读取预留）, 不动, 修改, 关键设计决策, 前端（修改 3） (+12 more)

### Community 532 - "Community 532"
Cohesion: 0.29
Nodes (10): 📚 API 文档, code:block7 (├── app/                  # 应用相关模型), Wujie-Jilin-Admin_Cloud, Wujie-Jilin-Admin_Cloud, 🛠️ 技术栈, 提交 Pull Request 前请确保：, 📁 目录结构, 📧 联系方式 (+2 more)
...
Cohesion: 0.18
Nodes (20): bool, int, str, add_face_entity(), add_face_image(), aliyun_oss_url(), create_face_db(), delete_face() (+12 more)
...
Cohesion: 0.20
Nodes (19): AsyncSession, int, PageRequest, RobotEventLogQueryParams, SysUser, AsyncSession, int, batch_delete_logs() (+11 more)

### Community 544 - "Community 544"
Cohesion: 0.07
Nodes (33): int, SnowflakeInfo, _current_millis(), generate_id(), parse_id(), Snowflake, SnowflakeConfig, int (+25 more)

### Community 545 - "Community 545"
Cohesion: 0.28
Nodes (14): int, MapInfo, str, _get_stub_for_addr(), notify_map_saved(), notify_map_saved_one(), MapService gRPC 客户端  封装 NotifyMapSaved / SearchMaps / SwitchMap 三个 RPC：  地址路由（ta, search_maps() (+6 more)

### Community 546 - "Community 546"
Cohesion: 0.11
Nodes (17): code:block1 (python -c "from app.grpc.generated.task import task_pb2, tas), operation 取值约定, 不接入重试基建, 不覆盖任务定义变更, 任务执行 gRPC 推送补全, 修改, 关键决策, 放 service 层而非 endpoint 层 (+9 more)
...
Cohesion: 0.20
Nodes (9): 人脸识别删除修复, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 551 - "Community 551"
Cohesion: 0.20
Nodes (9): 人脸识别人像预览路径修复, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 552 - "Community 552"
Cohesion: 0.20
Nodes (9): 人脸识别人像上传接口修复, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 553 - "Community 553"
Cohesion: 0.29
Nodes (10): 前端, 后端, 地图编辑器新增场景图片与扫图起始点, 地图编辑器新增场景图片与起始点位, 涉及范围, 状态, 相关文件, 约束与备注 (+2 more)

### Community 554 - "Community 554"
Cohesion: 0.20
Nodes (9): 2026-06-11 地图编辑器右侧卡片与机器人总览, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 555 - "Community 555"
Cohesion: 0.20
Nodes (9): 2026-06-11 地图编辑器机器人定位与绑定场景, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 556 - "Community 556"
Cohesion: 0.20
Nodes (9): 前端, 后端, 机器人配置迁移修复, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 557 - "Community 557"
Cohesion: 0.20
Nodes (9): 前端, 后端, 机器人参数配置选择机器人布局调整, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 558 - "Community 558"
Cohesion: 0.20
Nodes (9): 前端, 后端, 机器人场景绑定可空与存量库缺列修复, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 559 - "Community 559"
Cohesion: 0.20
Nodes (9): 前端, 后端, 场景地图 JSON 点位导入, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)
...
Cohesion: 0.22
Nodes (8): 2026-06-10 `/task` 新增任务按钮修复, 2026-06-10 历史任务操作入口调整, 3.2.4 F-04 任务管理, 关键业务规则, 变更记录, 实现范围, 用例清单, 需求概述

### Community 572 - "Community 572"
Cohesion: 0.18
Nodes (11): 1. 添加子模块, 2. 克隆带子模块的仓库, 3. 更新子模块内容, 4. 子模块使用注意事项, code:powershell (git status), code:powershell (git submodule add <proto仓库地址> protos), code:powershell (git submodule add https://example.com/your-org/your-protos.g), code:powershell (git clone --recurse-submodules <本项目仓库地址>) (+3 more)
...
Cohesion: 0.20
Nodes (9): CommonSearchParams, ConfigUpdateResponse, FaceRecognition, FaceRecognitionCreate, FaceRecognitionList, GrpcStatus, TestTTSRequest, TestWakeWordRequest (+1 more)
...
Cohesion: 0.20
Nodes (9): 任务固化场景地图字段, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)
...
Cohesion: 0.38
Nodes (11): AsyncSession, int, RobotModelQueryParams, RobotModel, build_query(), create(), delete(), get() (+3 more)
...
Cohesion: 0.33
Nodes (5): 内容, 实现方式, 时间, 涉及文件, 需求：场景地图新增功能增强
...
Cohesion: 0.20
Nodes (9): 任务管理页面白色卡片背景调整, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 631 - "Community 631"
Cohesion: 0.31
Nodes (21): AsyncSession, bool, int, str, batch_delete_configs(), batch_update_configs(), build_config_query(), _convert_value() (+13 more)

### Community 632 - "Community 632"
Cohesion: 0.15
Nodes (12): 关键业务规则, 前端, 后端（纯只读新增，无写入改动、无 DB 迁移、无 proto 变更）, 地图编辑器显示机器人实时位置 + 修复定位, 涉及范围, 状态, 相关历史记忆, 相关文件 (+4 more)

### Community 633 - "Community 633"
Cohesion: 0.33
Nodes (6): code:bash (cd backend && ./start_prod.sh), code:bash (ENVIR=prod gunicorn main:app -w 4 -k uvicorn.workers.Uvicorn), code:bash (# 使用 uvicorn 运行), 开发环境, 生产环境, 🚀 部署说明
...
Cohesion: 0.20
Nodes (9): 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期, 需求描述 (+1 more)

### Community 657 - "Community 657"
Cohesion: 0.20
Nodes (9): 前端, 后端, 校验错误提示词去中英文混合（用字段中文描述指代）, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 658 - "Community 658"
Cohesion: 0.40
Nodes (6): code:dockerfile (# 示例 Dockerfile), code:bash (# 创建新的迁移文件), code:bash (# 代码格式化), 代码规范, 🔧 开发指南, 数据库迁移
...
Cohesion: 0.40
Nodes (4): opt, result, string, ThemeColor
...
Cohesion: 0.33
Nodes (9): downgrade(), seed data (squashed DML baseline)  Revision ID: 0002 Revises: 0001 Create Da, _seed_0003(), _seed_0013_robot_status_backfill(), _seed_0014_fix_location_info_string(), _seed_0018_scene_map_nav_image(), _seed_0028_tts_speed_to_float(), _seed_0037_disable_task_sched_scan() (+1 more)

### Community 678 - "Community 678"
Cohesion: 0.60
Nodes (3): downgrade(), seed scene_map-editor and task menus  Revision ID: 0005_menus Revises: 0004_t, upgrade()

### Community 679 - "Community 679"
Cohesion: 0.60
Nodes (3): downgrade(), robot config tables + menus  Revision ID: 0006 Revises: 0005_menus Create Da, upgrade()

### Community 680 - "Community 680"
Cohesion: 0.12
Nodes (15): 任务 1：gRPC 配置, 任务 2：隐藏状态按钮, 任务 3：隐藏搜索框, 任务 4：修复删除机器人报错, 任务 5：修复编辑任务触发 scene:map:list, 关键决策, 前端, 后端 (+7 more)
...
Cohesion: 0.11
Nodes (35): _ddl_0001(), _ddl_0003(), _ddl_0004_task(), _ddl_0006(), _ddl_0007_robot_voice_config_robot_id(), _ddl_0008_robot_map_binding(), _ddl_0009_scene_map_start_point(), _ddl_0010_robot_event_log() (+27 more)
...
Cohesion: 0.27
Nodes (15): Any, int, str, timedelta, datetime, HTTPException, create_access_token(), create_refresh_token() (+7 more)

### Community 760 - "Community 760"
Cohesion: 0.23
Nodes (15): annotationAngleDeg, filteredAnnotations, filteredList, loadRobotList(), locateRobot(), pixelToMeter(), pointTypeOptions, robotLoading (+7 more)

### Community 768 - "Community 768"
Cohesion: 0.40
Nodes (3): img, result, url

### Community 853 - "Community 853"
Cohesion: 0.20
Nodes (9): 任务地图优先选择与点位删除联动, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 856 - "Community 856"
Cohesion: 0.15
Nodes (12): 关键业务规则, 创建机器人时默认启用唤醒词「小护小护」, 前端, 后端, 涉及范围, 状态, 相关历史记忆, 相关文件 (+4 more)

### Community 857 - "Community 857"
Cohesion: 0.14
Nodes (13): UC-A: settings 权限统一, UC-B: settings 滚动, UC-C: 表单展开/收起, UC-D: 点位多动作, UC-E: 独立执行记录表, 关键业务规则, 关键设计决策, 前端 (+5 more)

### Community 858 - "Community 858"
Cohesion: 0.18
Nodes (10): 前端, 后端, 地图编辑：删除点位仅在其已关联任务时弹窗确认, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+2 more)

### Community 859 - "Community 859"
Cohesion: 0.20
Nodes (9): 任务新增编辑必填校验, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 860 - "Community 860"
Cohesion: 0.20
Nodes (9): 任务运控动作下拉精简（移除击掌/拥抱/飞吻/动感光波）, 保留不改（历史数据兼容）, 前端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)
...
Cohesion: 0.20
Nodes (9): 任务启用禁用按钮修复, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 863 - "Community 863"
Cohesion: 0.20
Nodes (23): int, str, add_face_entity(), add_face_image(), create_face_db(), delete_face(), delete_face_entity(), _describe_aliyun_error() (+15 more)

### Community 864 - "Community 864"
Cohesion: 0.25
Nodes (8): code:powershell (uv sync), code:powershell (uv run python .\main.py), code:powershell (uv run python -c "import grpc_tools; print(grpc_tools.__file), code:powershell (git submodule update --init --recursive), ModuleNotFoundError: No module named 'grpc_tools', 修改 proto 后生成文件没有变化, 子模块目录为空, 常见问题

### Community 865 - "Community 865"
Cohesion: 0.14
Nodes (7): NavigationService, NavigationServiceServicer, NavigationServiceStub, 即时导航服务     商户开放 API（goto_point / navigate_route）调用，直接把目标点位下发到机器人 agent。     与, Constructor.          Args:             channel: A grpc.Channel., 即时导航服务     商户开放 API（goto_point / navigate_route）调用，直接把目标点位下发到机器人 agent。     与, 即时导航服务     商户开放 API（goto_point / navigate_route）调用，直接把目标点位下发到机器人 agent。     与

### Community 866 - "Community 866"
Cohesion: 0.12
Nodes (16): code:block1 (python -m py_compile modules/task/services/task_service.py), 不接 GrpcRetryService 重试队列, 任务定义变更 gRPC 推送（create/edit/delete + enable/disable）, 关键决策, 前端, 后端, 复用现有 TaskConfigClient，不新增 proto 字段, 放 service 层而非 endpoint 层 (+8 more)

### Community 867 - "Community 867"
Cohesion: 0.50
Nodes (4): 5. 注意导入路径修复, code:python (import map_pb2 as map__pb2), code:python (import app.grpc.generated.map.map_pb2 as map__pb2), code:python (prefix=f"app.grpc.generated.{module}.")

### Community 868 - "Community 868"
Cohesion: 0.50
Nodes (4): code:text (protos/user/user.proto), code:python (modules = ["map", "user"]), code:powershell (uv run python .\main.py), 新增 proto 模块

### Community 869 - "Community 869"
Cohesion: 0.60
Nodes (3): downgrade(), add operation-monitor menu  Revision ID: 0011_operation_monitor_menu Revises:, upgrade()
...
Cohesion: 0.09
Nodes (22): Any, RobotStatus, str, _coerce_update_at(), LocationInfoData, _normalize_location_info(), 机器人状态记录查询参数模型     用于状态记录列表分页查询时的筛选条件, 机器人状态记录查询参数模型     用于状态记录列表分页查询时的筛选条件 (+14 more)
...
Cohesion: 0.12
Nodes (27): AsyncSession, int, PageRequest, Request, RobotModelCreate, RobotModelQueryParams, RobotModelUpdate, SysUser (+19 more)

### Community 887 - "Community 887"
Cohesion: 0.15
Nodes (8): BatteryConfigService, BatteryConfigServiceServicer, BatteryConfigServiceStub, 电量报警阈值推送服务     调用方完成 DB 写入后调用，将最新电量阈值推送给机器人侧立即生效, Constructor.          Args:             channel: A grpc.Channel., 电量报警阈值推送服务     调用方完成 DB 写入后调用，将最新电量阈值推送给机器人侧立即生效, Missing associated documentation comment in .proto file., 电量报警阈值推送服务     调用方完成 DB 写入后调用，将最新电量阈值推送给机器人侧立即生效

### Community 902 - "Community 902"
Cohesion: 0.15
Nodes (8): FaceRecognitionService, FaceRecognitionServiceServicer, FaceRecognitionServiceStub, 人脸识别 TTS 库变更推送服务     调用方完成 DB 写入后调用，按 operation 推送人脸库增量变更：     create     新增人员, Constructor.          Args:             channel: A grpc.Channel., 人脸识别 TTS 库变更推送服务     调用方完成 DB 写入后调用，按 operation 推送人脸库增量变更：     create     新增人员, Missing associated documentation comment in .proto file., 人脸识别 TTS 库变更推送服务     调用方完成 DB 写入后调用，按 operation 推送人脸库增量变更：     create     新增人员

### Community 904 - "Community 904"
Cohesion: 0.15
Nodes (8): 行走速度配置推送服务     调用方完成 DB 写入后调用，将最新速度等级推送给机器人侧立即生效, Constructor.          Args:             channel: A grpc.Channel., 行走速度配置推送服务     调用方完成 DB 写入后调用，将最新速度等级推送给机器人侧立即生效, Missing associated documentation comment in .proto file., 行走速度配置推送服务     调用方完成 DB 写入后调用，将最新速度等级推送给机器人侧立即生效, SpeedConfigService, SpeedConfigServiceServicer, SpeedConfigServiceStub

### Community 905 - "Community 905"
Cohesion: 0.12
Nodes (12): 任务变更通知服务     调用方完成 DB 写入后调用，服务端按 operation 执行调度联动：     create     重新读取任务；若 ena, Constructor.          Args:             channel: A grpc.Channel., Constructor.          Args:             channel: A grpc.Channel., 任务变更通知服务     调用方完成 DB 写入后调用，服务端按 operation 执行调度联动：     create     重新读取任务；若 ena, 任务变更通知服务     调用方完成 DB 写入后调用，服务端按 operation 执行调度联动：     create     重新读取任务；若 ena, Missing associated documentation comment in .proto file., Missing associated documentation comment in .proto file., 任务变更通知服务     调用方完成 DB 写入后调用，服务端按 operation 执行调度联动：     create     重新读取任务；若 ena (+4 more)

### Community 906 - "Community 906"
Cohesion: 0.24
Nodes (13): Any, int, str, _build_point(), _get_stub_for_addr(), navigate_route(), navigate_to_point(), NavigationService gRPC 客户端  商户开放 API（goto_point / navigate_route）的即时导航下发，按 robot (+5 more)
...
Cohesion: 0.11
Nodes (30): colorNameMap, colorNames, colorPalettes, darkColorMap, getAntDColorPalette(), getAntDPaletteColorByIndex(), getHue(), getSaturation() (+22 more)

### Community 909 - "Community 909"
Cohesion: 0.15
Nodes (12): 前端, 后端, 后续补丁（2026-06-24 当天）, 后续补丁 2：上传后无法重新上传（2026-06-24 当天）, 场景地图主图上传接口独立权限化, 未改动, 涉及范围, 状态 (+4 more)

### Community 910 - "Community 910"
Cohesion: 0.17
Nodes (11): gRPC proto, 关键设计决策, 前端, 后端, 数据库迁移, 涉及范围, 状态, 相关文件 (+3 more)
...
Cohesion: 0.60
Nodes (5): AsyncSession, bool, _add_foreign_key(), ensure_robot_map_binding(), _has_robot_map_id()

### Community 914 - "Community 914"
Cohesion: 0.20
Nodes (9): 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期, 运行监控页面滚动 + 登录后底部栏隐藏 (+1 more)

### Community 915 - "Community 915"
Cohesion: 0.20
Nodes (9): 前后端生产环境启动脚本, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 916 - "Community 916"
Cohesion: 0.20
Nodes (9): 前端, 后端, 地图编辑器电子围栏（Geofence）, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 917 - "Community 917"
Cohesion: 0.20
Nodes (9): 前端, 后端, 涉及范围, 状态, 相关文件, 移除播报类型任务的播报次数配置, 约束与备注, 记录日期 (+1 more)

### Community 918 - "Community 918"
Cohesion: 0.20
Nodes (9): voice.proto 拆分为唤醒词 + 语音合成两个 RPC, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 919 - "Community 919"
Cohesion: 0.20
Nodes (9): 前端, 后端, 唤醒词测试按钮右侧显示模拟回应话术 + proto 新增测试 RPC, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 920 - "Community 920"
Cohesion: 0.20
Nodes (9): 前端, 后端, 地图编辑器删除地图后自动切换到第一个, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 921 - "Community 921"
Cohesion: 0.14
Nodes (13): 兜底规则, 关键决策, 分流规则, 前端, 参数配置 gRPC 调用从 robot.grpc_config 取地址, 后端, 涉及范围, 状态 (+5 more)

### Community 922 - "Community 922"
Cohesion: 0.35
Nodes (10): current_pid(), ENVIR, is_running(), logs(), restart(), start(), status(), stop() (+2 more)

### Community 923 - "Community 923"
Cohesion: 0.14
Nodes (13): FaceDbList, FaceDetectItem, FaceDetectParams, FaceDetectResult, FaceEntity, FaceEntityDetail, FaceEntityFace, FaceEntityList (+5 more)

### Community 924 - "Community 924"
Cohesion: 0.17
Nodes (11): 保存地图时默认推送所有机器人（不再按 map_id 过滤）, 前端, 后端, 核心决策, 涉及范围, 状态, 相关历史记忆, 相关文件 (+3 more)

### Community 925 - "Community 925"
Cohesion: 0.20
Nodes (9): 前端, 后端, 地图编辑器「扫图起始点」返回点与 start_point 解耦, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 926 - "Community 926"
Cohesion: 0.15
Nodes (12): scheduler 按钮 8 条重命名映射, 关键决策, 前端, 后端, 按钮权限来源盘点, 涉及范围, 状态, 相关文件 (+4 more)

### Community 927 - "Community 927"
Cohesion: 0.22
Nodes (8): 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期, 需求描述

### Community 930 - "Community 930"
Cohesion: 0.18
Nodes (10): code:yaml (image: map.png), 前端, 后端, 地图编辑器新增场景：上传 ROS yaml 配置解析分辨率与起始点, 涉及范围, 状态, 相关文件, 约束与备注 (+2 more)
...
Cohesion: 0.15
Nodes (12): 前端, 后端, 唤醒词保存 gRPC 改为同时推 middleware + agent, 涉及范围, 状态, 相关历史记忆, 相关文件, 约束与备注 (+4 more)

### Community 1018 - "Community 1018"
Cohesion: 0.17
Nodes (11): 不放宽 object / path, 任务管理选地图后加载点位 403, 关键决策, 复用 task:list，沿用上次策略, 涉及范围, 状态, 相关文件, 记录日期 (+3 more)

### Community 1019 - "Community 1019"
Cohesion: 0.20
Nodes (9): 前端, 历史任务列表按结束时间倒序, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 1020 - "Community 1020"
Cohesion: 0.24
Nodes (12): str, Config, autogenerate_and_upgrade(), generate_removal_and_upgrade(), _get_alembic_config(), get_current_head(), 获取 alembic 配置，自动定位 alembic.ini, 获取当前 alembic head revision (+4 more)

### Community 1022 - "Community 1022"
Cohesion: 0.06
Nodes (101): AsyncSession, bool, int, PageRequest, Request, str, SysFileQueryParams, SysUser (+93 more)

### Community 1023 - "Community 1023"
Cohesion: 0.18
Nodes (10): 关键决策, 涉及范围, 状态, 相关历史记忆, 相关文件, 移除本服务定时调度 + 启动任务改为纯 gRPC, 范围外 / 影响, 需求 1：关闭定时调度处理（字段保留） (+2 more)

### Community 1024 - "Community 1024"
Cohesion: 0.17
Nodes (11): 关键决策, 前端, 后端, 地图同步 gRPC 改走机器人 middleware 地址, 涉及范围, 状态, 相关文件, 约束与备注 (+3 more)

### Community 1025 - "Community 1025"
Cohesion: 0.18
Nodes (10): 关键决策, 前端, 后端, 机器人 gRPC 配置新增 ros 选项, 涉及范围, 状态, 相关文件, 约束与备注 (+2 more)

### Community 1026 - "Community 1026"
Cohesion: 0.18
Nodes (10): 与既有"机器人人脸"功能的区别, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+2 more)

### Community 1027 - "Community 1027"
Cohesion: 0.18
Nodes (10): 关键决策, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 菜单管理：路径前缀继承、目录级联、移到根目录修复 (+2 more)

### Community 1028 - "Community 1028"
Cohesion: 0.18
Nodes (10): 前端, 后端, 涉及范围, 状态, 相关历史记忆, 相关文件, 约束与备注, 记录日期 (+2 more)

### Community 1029 - "Community 1029"
Cohesion: 0.13
Nodes (14): 2026-07-10 运行监控实时视频接入 LiveKit, code:env (LIVEKIT__ENABLED=true), 业务规则, 仍需确认, 关键设计, 后续注意, 已修复/已增强, 已修复/已增强：切换机器人或关闭视频流时偶发异常 (+6 more)

### Community 1031 - "Community 1031"
Cohesion: 0.15
Nodes (12): map.proto 新增 SwitchMap 方法：切换机器人当前地图, proto（`backend/grpc/` 子仓库）, 关键业务规则, 契约设计（与用户确认）, 平台后端, 涉及范围, 状态, 相关历史记忆 (+4 more)

### Community 1033 - "Community 1033"
Cohesion: 0.29
Nodes (10): 前端, 后端, 涉及范围, 状态, 登录后默认跳转到权限列表的第一个页面, 登录后默认跳转到权限列表的第一个页面（跳过 /home 仪表盘）, 相关文件, 约束与备注 (+2 more)

### Community 1034 - "Community 1034"
Cohesion: 0.20
Nodes (9): 任务运控动作选项更新 & 允许点位动作为空, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 1035 - "Community 1035"
Cohesion: 0.20
Nodes (9): 前端, 后端, 地图编辑器点位落障碍物拦截, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 1036 - "Community 1036"
Cohesion: 0.20
Nodes (9): 前端, 后端, 商户管理 + 商户开放 API, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 1037 - "Community 1037"
Cohesion: 0.22
Nodes (8): 关键决策, 机器人管理卡片按钮补齐权限控制, 权限码映射, 涉及范围, 状态, 约束与备注, 记录日期, 需求描述

### Community 1038 - "Community 1038"
Cohesion: 0.15
Nodes (12): ApiCredentials, CallLog, CallLogDetail, CallLogList, CallLogSearchParams, CommonSearchParams, Merchant, MerchantCreate (+4 more)

### Community 1039 - "Community 1039"
Cohesion: 0.31
Nodes (8): str, Fernet, decrypt(), encrypt(), _get_fernet(), 根据配置 passphrase 派生 Fernet 实例（进程级缓存）, 加密明文，返回 Fernet token 字符串, 解密 Fernet token，返回明文；token 非法时抛 ValueError

### Community 1040 - "Community 1040"
Cohesion: 0.29
Nodes (9): int, str, broadcast_task_changed(), _get_stub_for_addr(), notify_task_changed(), RouteTaskService gRPC 客户端  任务运行时变更通知，按任务关联的 robot 列表逐个推送到 agent： - notify_task_c, object, RouteTaskServiceStub (+1 more)

### Community 1041 - "Community 1041"
Cohesion: 0.62
Nodes (4): apiSecret, copyText(), legacyCopy(), maskedSecret
...
Cohesion: 0.22
Nodes (8): 任务管理：重复周期全选 7 天报「字段长度不够」, 关键实现, 根因, 相关文件, 约束与备注, 记录日期, 踩坑：revision id 超长, 需求描述

### Community 1128 - "Community 1128"
Cohesion: 0.60
Nodes (4): extractRobotPoint(), isFiniteNumber(), RobotLocationSource, RobotPoint

### Community 1129 - "Community 1129"
Cohesion: 0.15
Nodes (12): 前端, 后端, 后续：地图编辑器同步同一套缩放逻辑（2026-07-01）, 后续：机器人标记朝向角度 + 名称固定下方（2026-07-01）, 涉及范围, 状态, 相关历史记忆, 相关文件 (+4 more)

### Community 1146 - "Community 1146"
Cohesion: 0.17
Nodes (11): OpenAPI 导航补全 dedicated gRPC, 关键决策, 前端, 后端, 改动总览, 涉及范围, 状态, 相关文件 (+3 more)

### Community 1148 - "Community 1148"
Cohesion: 0.20
Nodes (9): 参数配置 gRPC target 调整：唤醒词测试 + 电量阈值改走 agent, 涉及范围, 状态, 相关历史记忆, 相关文件, 约束与备注, 记录日期, 调整后完整 target 对照 (+1 more)

### Community 1149 - "Community 1149"
Cohesion: 0.20
Nodes (9): 任务点位动作列表可为空, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 1150 - "Community 1150"
Cohesion: 0.20
Nodes (9): 任务绑定机器人与场景一致性提示, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 1151 - "Community 1151"
Cohesion: 0.15
Nodes (12): 任务定时配置真正落库（schedule_date / schedule_start_time）, 关键业务规则, 前端, 后端, 涉及范围, 状态, 相关历史记忆, 相关文件 (+4 more)
...
Cohesion: 0.17
Nodes (36): AsyncSession, int, IpBlacklistCreateRequest, IpBlacklistQueryParams, PageRequest, SysUser, AsyncSession, datetime (+28 more)

### Community 1196 - "Community 1196"
Cohesion: 0.18
Nodes (10): 前端, 后端, 商户开放 API 新增场景/任务/点位列表接口, 接口契约, 涉及范围, 状态, 相关文件, 约束与备注 (+2 more)
...
Cohesion: 0.41
Nodes (6): Any, int, str, Any, int, str

### Community 1201 - "Community 1201"
Cohesion: 0.22
Nodes (8): code:block1 (添加人脸图片失败: Error: InvalidImage.NotFoundFace code: 404, [pk=..), 参数配置·人脸识别 阿里云错误信息友好化解析, 方案, 状态, 相关文件, 约束与备注, 记录日期, 需求描述

### Community 1202 - "Community 1202"
Cohesion: 0.15
Nodes (12): code:block1 (new_pixelX = old_pixelX + new_start_x - old_start_x), code:block2 (new_pixelX = (old_pixelX * old_resolution + old_start_x - ne), 前端, 后端, 地图编辑：编辑扫图起始点时按差值平移子元素, 涉及范围, 状态, 相关文件 (+4 more)

### Community 1203 - "Community 1203"
Cohesion: 0.25
Nodes (6): MappedAsDataclass 模型的 Optional 字段必须写 default=None, 典型案例, 如何排查, 字段顺序, 正确写法, 约束

### Community 1204 - "Community 1204"
Cohesion: 0.25
Nodes (7): 前端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期, 需求描述

### Community 1205 - "Community 1205"
Cohesion: 0.20
Nodes (9): 前端, 参数配置·人脸识别TTS 人像上传限制（对齐阿里云 facebody 规格）, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 1206 - "Community 1206"
Cohesion: 0.20
Nodes (9): 前端, 后端, 日志管理三页面增加导出 Excel（复用异步导出任务体系 + 顶栏下载箱）, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)
...
Cohesion: 0.30
Nodes (18): AsyncSession, bool, int, Merchant, MerchantCreate, MerchantUpdate, str, ApiKeyService (+10 more)
...
Cohesion: 0.09
Nodes (88): MerchantQueryParams, Select, datetime, Robot, SceneMapAnnotation, Task, TaskExecutionRecord, RobotEventLogQueryParams (+80 more)

### Community 1227 - "Community 1227"
Cohesion: 0.18
Nodes (10): 关键决策, 前端, 后端, 播报任务多机器人启动改为尽力下发（无需全部在线）, 涉及范围, 状态, 相关历史记忆, 相关文件 (+2 more)

### Community 1228 - "Community 1228"
Cohesion: 0.17
Nodes (11): 前端, 后端, 地图保存/切换 gRPC 增加 agent 端推送（middleware + agent 双推）, 核心决策, 涉及范围, 状态, 相关历史记忆, 相关文件 (+3 more)

### Community 1229 - "Community 1229"
Cohesion: 0.09
Nodes (27): ChartHooks, ECOption, useEcharts(), useSvgIcon(), useRouterPush(), [provideMixMenuContext, useMixMenuContext], useMenu(), useMixMenu() (+19 more)
...
Cohesion: 0.25
Nodes (15): int, str, UploadFile, add_face_entity(), add_face_image(), create_face_db(), delete_face(), delete_face_entity() (+7 more)
...
Cohesion: 0.20
Nodes (9): 涉及范围, 状态, 相关历史记忆, 相关文件, 约束与备注, 记录日期, 语音配置 gRPC target 统一回归 middleware, 调整后完整 target 对照 (+1 more)
...
Cohesion: 0.07
Nodes (31): 1\.1 平台定位, 1\.2 环境与账户, 1\.3 接入机器人设备, 2\.1 登录系统, 2\.2 主界面布局, 3\.1 机器人管理, 3\.2\.1 工作流程, 3\.2\.2 编辑画布交互 (+23 more)

### Community 1237 - "Community 1237"
Cohesion: 0.33
Nodes (5): 2026-07-03 场景地图新增/编辑核心字段必填, 不在表单中填写的字段, 必填字段, 涉及文件, 需求
...
Cohesion: 0.11
Nodes (17): 2026-07-10 本地 .log 日志按日期滚动, 2026-07-30 修复：多 worker 下 info.log 不滚动, code:bash (sudo cp deploy/logrotate/smilex-cloud /etc/logrotate.d/smile), code:bash (sudo logrotate -d /etc/logrotate.d/smilex-cloud), code:bash (sudo bash -c 'echo "test" >> /opt/smilex-cloud/backend/logs/), code:bash (sudo logrotate -f /etc/logrotate.d/smilex-cloud), 修复（统一交给 logrotate，与 smilex-cloud.log 一致）, 修改文件 (+9 more)

### Community 1240 - "Community 1240"
Cohesion: 0.18
Nodes (10): 关键决策, 前端, 后端, 地图编辑器机器人切换绑定地图时检查进行中任务, 涉及范围, 状态, 相关历史记忆, 相关文件 (+2 more)

### Community 1250 - "Community 1250"
Cohesion: 0.18
Nodes (10): 关键决策, 前端, 后端, 巡逻任务启动校验任务地图与机器人地图一致, 涉及范围, 状态, 相关历史记忆, 相关文件 (+2 more)

### Community 1253 - "Community 1253"
Cohesion: 0.40
Nodes (4): ExportTask, ExportTaskList, ExportTaskStatus, ExportTaskSubmit

### Community 1254 - "Community 1254"
Cohesion: 0.08
Nodes (20): useAuthStore, clearAuthStorage(), getToken(), useAuth(), connected, lastMessage, useWebSocketNotification(), permission (+12 more)
...
Cohesion: 0.19
Nodes (21): AsyncSession, int, SceneMapPathCreate, SceneMapPathUpdate, SysUser, AsyncSession, int, SceneMapPathCreate (+13 more)
...
Cohesion: 0.13
Nodes (14): RobotSimpleResponse 字段权衡, 关键决策, 前端, 后端, 已解决的不再动, 沿用 /all 模式而非 require_any_permission, 涉及范围, 状态 (+6 more)
...
Cohesion: 0.22
Nodes (8): 前置条件：, 前置条件：, 操作步骤：, 操作步骤：, 方案A：电脑连接, 方案B：显示屏连接, 方案总览, 语音背包wifi配网操作指南

### Community 1294 - "Community 1294"
Cohesion: 0.15
Nodes (8): 视频监控启停推送服务     由平台侧调用机器人 middleware，实时下发视频监控启动 / 停止指令（fire-and-forget，不入重试队列）。, Constructor.          Args:             channel: A grpc.Channel., 视频监控启停推送服务     由平台侧调用机器人 middleware，实时下发视频监控启动 / 停止指令（fire-and-forget，不入重试队列）。, Missing associated documentation comment in .proto file., 视频监控启停推送服务     由平台侧调用机器人 middleware，实时下发视频监控启动 / 停止指令（fire-and-forget，不入重试队列）。, VideoMonitoringService, VideoMonitoringServiceServicer, VideoMonitoringServiceStub
...
Cohesion: 0.17
Nodes (11): 关联记忆, 关键决策（已与用户确认）, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注 (+3 more)

### Community 1297 - "Community 1297"
Cohesion: 0.14
Nodes (13): 两种修法各司其职（沿用项目既有模式）, 为何不直接放宽 `/robot/manage/list`, 修复跨权限调用 robot:manage:list 接口, 关键决策, 前端, 后端, 涉及范围, 状态 (+5 more)

### Community 1299 - "Community 1299"
Cohesion: 0.20
Nodes (9): 任务类型新增「即时」(instant), 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)
...
Cohesion: 0.11
Nodes (19): ASGIApp, str, WebSocket, int, Token, int, get_current_tenant_id(), 设置当前请求的租户ID，返回 token 用于重置 (+11 more)
...
Cohesion: 0.29
Nodes (6): 商户开放 API 动作类接口增加机器人在线校验, 实现, 影响范围, 涉及接口, 相关文件, 需求

### Community 1307 - "Community 1307"
Cohesion: 0.25
Nodes (7): 2026-07-16 视频监控叠加状态信息（时间/分辨率/帧率/全屏）, 业务规则, 关联记忆, 关键设计, 涉及文件, 记录日期, 需求

### Community 1313 - "Community 1313"
Cohesion: 0.20
Nodes (9): 2026-07-14 地图编辑器无编辑权限时只读, code:js (if (!hasAuth('scene:map-editor:edit')) {), 右键菜单, 实现, 机器人切换地图, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 1314 - "Community 1314"
Cohesion: 0.29
Nodes (6): 商户开放 API 新增机器人列表查询接口, 实现, 影响范围, 接口契约, 相关文件, 需求

### Community 1315 - "Community 1315"
Cohesion: 0.20
Nodes (9): 分页参数脏值防御性收敛 + OptionalIntField 错误中文化, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)
...
Cohesion: 0.25
Nodes (7): 2026-07-20 机器人事件日志 event_status 枚举值重命名, 关键实现, 决策, 涉及文件, 约束与备注, 记录日期, 需求

### Community 1318 - "Community 1318"
Cohesion: 0.15
Nodes (12): code:ts (} else {), 修复, 备注, 核心修复（解决 3×3 问题）, 根因分析, 相关历史记忆, 相关文件, 记录日期 (+4 more)

### Community 1319 - "Community 1319"
Cohesion: 0.33
Nodes (5): 2026-07-31 deploy.sh 部署流程修复, 修改文件, 关键事实, 注意, 背景
...
Cohesion: 0.25
Nodes (16): AsyncSession, bool, int, str, get_redis_util(), 获取Redis工具实例     Returns:         RedisUtil: Redis工具实例, ResponsePageDataModel, _collect_online_sessions() (+8 more)
...
Cohesion: 0.14
Nodes (13): get_user_routes 改 selectinload 修复权限并集丢失, selectinload 而非给查询加 ignore_tenant, 与上一条软删过滤修复的关系, 关键决策, 后端, 当前 dev/prod 关着多租户, 根因, 涉及范围 (+5 more)

### Community 1336 - "Community 1336"
Cohesion: 0.25
Nodes (7): 关键实现, 机器人事件日志：搜索 warning 非法值 + 导出去事件类型/状态对齐/时间东八区, 根因, 相关文件, 约束与备注, 记录日期, 需求描述

### Community 1337 - "Community 1337"
Cohesion: 0.20
Nodes (9): 2026-07-11 实时监控机器人在线状态刷新, 业务规则, 关联记忆, 关键设计, 前端, 后端, 涉及文件, 记录日期 (+1 more)

### Community 1339 - "Community 1339"
Cohesion: 0.20
Nodes (9): 前端, 后端, 执行详情触发源补充「语音输入」「手动恢复」, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 1341 - "Community 1341"
Cohesion: 0.20
Nodes (9): 任务执行/历史列表与执行详情补齐「即时」任务类型, 前端, 后端, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)
...
Cohesion: 0.15
Nodes (12): gRPC 重试任务置 dead 时 next_retry_at 不能置 NULL, 不改 schema，只删一行, 与前序修复的关系, 关键决策, 后端, 根因, 涉及范围, 状态 (+4 more)

### Community 1362 - "Community 1362"
Cohesion: 0.15
Nodes (12): getPermissions 路由/按钮查询补软删除过滤, parent_map 也过滤, 关键决策, 只加过滤，不改「显式分配」语义, 后端, 根因（为何按钮「查全」是另一回事，本次只修脏数据）, 涉及范围, 状态 (+4 more)

### Community 1398 - "Community 1398"
Cohesion: 0.33
Nodes (5): 取消任务执行记录 task_id 外键约束, 实现, 影响范围, 背景, 需求

### Community 1399 - "Community 1399"
Cohesion: 0.33
Nodes (5): 商户开放 API 路径取消 /admin 前缀, 实现, 影响范围, 背景, 需求

### Community 1400 - "Community 1400"
Cohesion: 0.33
Nodes (5): 2026-07-10 机器人事件日志与实时告警样式调整, 关键实现, 涉及文件, 维护说明, 需求

### Community 1430 - "Community 1430"
Cohesion: 0.35
Nodes (10): bool, bytes, str, build_string_to_sign(), decrypt_secret(), encrypt_secret(), generate_api_key(), generate_api_secret() (+2 more)

### Community 1431 - "Community 1431"
Cohesion: 0.25
Nodes (7): 2026-07-11 视频监控 Tab 切换机器人时先关旧再开新, 业务规则, 关联记忆, 关键设计, 涉及文件, 记录日期, 需求

### Community 1432 - "Community 1432"
Cohesion: 0.20
Nodes (9): 前端, 后端, 机器人打招呼模式（greeting_mode）, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 1438 - "Community 1438"
Cohesion: 0.26
Nodes (13): AsyncSession, bool, int, LoginLogQueryParams, str, 系统登录日志表     记录用户登录尝试（成功和失败）, batch_delete_logs(), build_login_log_query() (+5 more)
...
Cohesion: 0.18
Nodes (10): 保存地图 gRPC 推送由串行改为有界并发, 前端, 后端, 收益与边界, 方案, 涉及范围, 状态, 相关文件 (+2 more)

### Community 1441 - "Community 1441"
Cohesion: 0.18
Nodes (10): 2026-07-14 新增地图返回点创建下沉到后端, code:block1 (fetchCreateSceneMap → switchMap(loadMap) → editor.addAnnotat), 前端, 原链路, 后端, 实现, 相关文件, 约束与备注 (+2 more)

### Community 1442 - "Community 1442"
Cohesion: 0.18
Nodes (40): CustomErrorCode, AsyncSession, bool, login(), Request, str, SysUser, SysUserPasswordUpdate (+32 more)

### Community 1443 - "Community 1443"
Cohesion: 0.36
Nodes (9): AsyncSession, int, TaskLogQueryParams, batch_delete_logs(), build_log_query(), clear_logs(), get_log(), SysScheduledTaskLog (+1 more)

### Community 1444 - "Community 1444"
Cohesion: 0.05
Nodes (64): int, BaseReqEntity, FaceDbCreate, FaceDbListResponse, FaceDetectItem, FaceDetectResponse, FaceEntityCreate, FaceEntityDetail (+56 more)

### Community 1445 - "Community 1445"
Cohesion: 0.29
Nodes (6): 任务管理：任务类型播报→巡逻时清空地图与机器人选择, 关键实现, 相关文件, 约束与备注, 记录日期, 需求描述

### Community 1446 - "Community 1446"
Cohesion: 0.20
Nodes (10): ConfigUpdateResponse, 参数配置保存响应（speed/battery 等不返回完整 ORM 的场景，仅携带 grpc_status）, 视频监控启停请求（robot_id 走 path）, 参数配置保存响应（speed/battery 等不返回完整 ORM 的场景，仅携带 grpc_status）, 视频监控启停请求（robot_id 走 path）, 视频监控启停请求（robot_id 走 path）, 参数配置保存响应（speed/battery 等不返回完整 ORM 的场景，仅携带 grpc_status）, 参数配置保存响应（speed/battery 等不返回完整 ORM 的场景，仅携带 grpc_status） (+2 more)

### Community 1447 - "Community 1447"
Cohesion: 0.25
Nodes (7): 2026-07-14 角色编辑重名校验, 实现, 涉及文件, 约束与备注, 背景 / 根因, 记录日期, 需求

### Community 1448 - "Community 1448"
Cohesion: 0.33
Nodes (6): confirmAndRemoveElement(), getElementNameAndKind(), handleContextMenuSelect(), handleDeleteKeyDown(), handleRemoveElement(), isInputElementFocused()

### Community 1449 - "Community 1449"
Cohesion: 0.29
Nodes (6): 2026-07-20 实时告警数量标签改灰色, 关键实现, 涉及文件, 约束与备注, 记录日期, 需求

### Community 1450 - "Community 1450"
Cohesion: 0.39
Nodes (8): SysUser, create_mcp_tool(), get_mcp_status(), list_mcp_routes(), list_mcp_tools(), start_mcp_server(), stop_mcp_server(), test_mcp_tool()
...
Cohesion: 0.67
Nodes (3): confirmAddScene(), confirmSceneSubmit(), getScaledStartPoint()

### Community 1462 - "Community 1462"
Cohesion: 0.67
Nodes (3): handleOpenAddScene(), handleOpenEditScene(), resetSceneForm()

### Community 1464 - "Community 1464"
Cohesion: 0.38
Nodes (6): Request, str, get_real_client_ip(), _normalize_ip(), 安全获取客户端真实IP，处理反向代理场景，防止IP伪造, 安全获取客户端真实IP，处理反向代理场景，防止IP伪造

### Community 1465 - "Community 1465"
Cohesion: 0.33
Nodes (5): 2026-07-17 实时告警事件状态前置显示, 关键实现, 涉及文件, 约束与备注, 需求

### Community 1467 - "Community 1467"
Cohesion: 0.17
Nodes (11): CommonSearchParams, DistributionItem, IntentType, SessionDetail, SessionList, SessionRecord, SessionSearchParams, SessionStatus (+3 more)

### Community 1470 - "Community 1470"
Cohesion: 0.20
Nodes (9): 唤醒词测试 / 语音合成测试推送改走机器人 agent, 涉及范围, 状态, 相关历史记忆, 相关文件, 约束与备注, 记录日期, 调整后完整 target 对照 (+1 more)

### Community 1471 - "Community 1471"
Cohesion: 0.40
Nodes (4): containerRect, el, elRect, html

### Community 1472 - "Community 1472"
Cohesion: 0.18
Nodes (10): DB 迁移, 前端, 后端, 枚举约定（外部写入方需遵守）, 涉及范围, 状态, 约束与备注, 记录日期 (+2 more)

### Community 1474 - "Community 1474"
Cohesion: 0.22
Nodes (9): 语音配置 gRPC 客户端（唤醒词 + TTS 音色/语速/音量）      唤醒词走 middleware，TTS 走 agent。, 语音配置 gRPC 客户端（唤醒词 + TTS 音色/语速/音量）      唤醒词保存(NotifyWakeWordChanged)走 middlewar, 语音配置 gRPC 客户端（唤醒词 + TTS 音色/语速/音量）      语音合成保存/测试与唤醒词测试统一走 middleware。, 语音配置 gRPC 客户端（唤醒词 + TTS 音色/语速/音量）      保存配置（notify_wake_word / notify_tts）走 mi, 语音配置 gRPC 客户端（唤醒词 + TTS 音色/语速/音量）      唤醒词保存(NotifyWakeWordChanged)走 middlewar, 语音配置 gRPC 客户端（唤醒词 + TTS 音色/语速/音量）      保存唤醒词（notify_wake_word）同时推 middleware +, 语音配置 gRPC 客户端（唤醒词 + TTS 音色/语速/音量）      保存唤醒词（notify_wake_word）同时推 middleware +, 语音配置 gRPC 客户端（唤醒词 + TTS 音色/语速/音量） (+1 more)

### Community 1490 - "Community 1490"
Cohesion: 0.25
Nodes (8): 更新机器人行走速度等级（参数配置专用，权限 robot:config:edit）, 更新机器人行走速度等级（参数配置专用，权限 robot:config:edit）, 更新机器人行走速度等级（参数配置专用，权限 robot:config:edit）, 更新机器人行走速度等级（参数配置专用，权限 robot:config:edit）, 更新机器人行走速度等级（参数配置专用，权限 robot:config:edit）, 更新机器人行走速度等级（参数配置专用，权限 robot:config:edit）, 更新机器人行走速度等级（参数配置专用，权限 robot:config:edit）, update_speed_level()

### Community 1491 - "Community 1491"
Cohesion: 0.22
Nodes (8): 关键实现, 机器人事件日志：导出「事件内容」从 JSON 提取 message, 相关文件, 约束与备注, 背景, 记录日期, 需求描述, 验证

### Community 1492 - "Community 1492"
Cohesion: 0.25
Nodes (8): 更新机器人电量报警阈值（参数配置专用，权限 robot:config:edit）, 更新机器人电量报警阈值（参数配置专用，权限 robot:config:edit）, 更新机器人电量报警阈值（参数配置专用，权限 robot:config:edit）, 更新机器人电量报警阈值（参数配置专用，权限 robot:config:edit）, 更新机器人电量报警阈值（参数配置专用，权限 robot:config:edit）, 更新机器人电量报警阈值（参数配置专用，权限 robot:config:edit）, 更新机器人电量报警阈值（参数配置专用，权限 robot:config:edit）, update_battery_threshold()

### Community 1495 - "Community 1495"
Cohesion: 0.14
Nodes (13): 前端, 前端：保存按钮顶层互斥锁（源头）, 后端, 后端：进程内 TTL 去重门（兜底）, 方案（前后端都加）, 机器人配置类 gRPC 推送 1s 内重复 —— 前后端双重防重, 根因, 涉及范围 (+5 more)
...
Cohesion: 0.25
Nodes (8): 测试唤醒词（调用 gRPC TestWakeWord，让机器人按当前唤醒词模拟一次响应）      测试按钮失败时直接返回 fail，不入重试队列（实时语义，用, 测试唤醒词（调用 gRPC TestWakeWord，让机器人按当前唤醒词模拟一次响应）      测试按钮失败时直接返回 fail，不入重试队列（实时语义，用, 测试唤醒词（调用 gRPC TestWakeWord，让机器人按当前唤醒词模拟一次响应）      测试按钮失败时直接返回 fail，不入重试队列（实时语义，用, 测试唤醒词（调用 gRPC TestWakeWord，让机器人按当前唤醒词模拟一次响应）      测试按钮失败时直接返回 fail，不入重试队列（实时语义，用, 测试唤醒词（调用 gRPC TestWakeWord，让机器人按当前唤醒词模拟一次响应）      测试按钮失败时直接返回 fail，不入重试队列（实时语义，用, 测试唤醒词（调用 gRPC TestWakeWord，让机器人按当前唤醒词模拟一次响应）      测试按钮失败时直接返回 fail，不入重试队列（实时语义, 测试唤醒词（调用 gRPC TestWakeWord，让机器人按当前唤醒词模拟一次响应）      测试按钮失败时直接返回 fail，不入重试队列（实时语义, test_wake_word()

### Community 1500 - "Community 1500"
Cohesion: 0.11
Nodes (22): getLogoSvg(), setupLoading(), getRgb(), presetSoybeanAdmin(), overrideThemeSettings, themeSettings, addThemeVarsToGlobal(), createThemePaletteColors() (+14 more)

### Community 1501 - "Community 1501"
Cohesion: 0.25
Nodes (8): 测试TTS语音合成（调用 gRPC TestTTSConfig，按指定参数播报测试文本）      测试按钮失败时直接返回 fail，不入重试队列（实时语义，用, 测试TTS语音合成（调用 gRPC TestTTSConfig，按指定参数播报测试文本）      测试按钮失败时直接返回 fail，不入重试队列（实时语义，用, 测试TTS语音合成（调用 gRPC TestTTSConfig，按指定参数播报测试文本）      测试按钮失败时直接返回 fail，不入重试队列（实时语义，用, 测试TTS语音合成（调用 gRPC TestTTSConfig，按指定参数播报测试文本）      测试按钮失败时直接返回 fail，不入重试队列（实时语义，用, 测试TTS语音合成（调用 gRPC TestTTSConfig，按指定参数播报测试文本）      测试按钮失败时直接返回 fail，不入重试队列（实时语义，用, 测试TTS语音合成（调用 gRPC TestTTSConfig，按指定参数播报测试文本）      测试按钮失败时直接返回 fail，不入重试队列（实时语义, 测试TTS语音合成（调用 gRPC TestTTSConfig，按指定参数播报测试文本）      测试按钮失败时直接返回 fail，不入重试队列（实时语义, test_tts()

### Community 1502 - "Community 1502"
Cohesion: 0.23
Nodes (12): Any, bool, float, int, str, _cleanup_expired(), _make_key(), gRPC 实时推送进程内去重门  职责： 对「同一机器人 + 同一方法 + 字节级完全相同的载荷」在短窗口内（默认 1s）只推一次， 从源头压掉双击 / 多标签 (+4 more)
...
Cohesion: 0.37
Nodes (12): float, int, str, _apply_fence_mask(), _draw_object(), _ellipse_points(), _parse_points(), _push_map_to_one() (+4 more)

### Community 1506 - "Community 1506"
Cohesion: 0.33
Nodes (5): 任务管理 · 任务列表「复制」按钮 + 任务名唯一性校验, 关键决策, 实现范围, 需求概述, 验证
...
Cohesion: 0.19
Nodes (21): AsyncSession, int, SceneMapObjectCreate, SceneMapObjectUpdate, SysUser, AsyncSession, int, SceneMapObjectCreate (+13 more)

### Community 1511 - "Community 1511"
Cohesion: 0.39
Nodes (15): AsyncSession, bool, int, str, cleanup_expired_viewers(), _cleanup_room(), close_viewer(), _generate_token() (+7 more)
...
Cohesion: 0.24
Nodes (9): ApiStatsQueryParams, AsyncSession, ApiStatsQueryParams, ApiStatsResponse, DiskInfo, get_api_stats(), MemoryInfo, 从操作日志中按时间窗口聚合API响应时间和错误率 (+1 more)
...
Cohesion: 0.20
Nodes (9): 前端, 后端, 机器人卡片「服务器自启状态 + 重启」, 涉及范围, 状态, 相关文件, 约束与备注, 记录日期 (+1 more)

### Community 1579 - "Community 1579"
Cohesion: 0.22
Nodes (13): Any, ConfigType, str, _convert_value(), _ensure_cache(), _fallback_defaults(), force_refresh(), get() (+5 more)

### Community 1581 - "Community 1581"
Cohesion: 0.57
Nodes (6): Robot, str, get_slot_status(), _panel_ids(), 面板要求的标识：robot_id + serial_number，每次必带且必须同一台, restart_slot()
...
Cohesion: 0.31
Nodes (7): AsyncSession, int, _bytes_to_gb(), _bytes_to_mb(), _collect_system_metrics(), get_api_stats(), MonitorService
...
Cohesion: 0.40
Nodes (9): AsyncSession, int, SceneMapAnnotationCreate, SceneMapAnnotationUpdate, SysUser, create_annotation(), delete_annotation(), get_annotation_list() (+1 more)
...
Cohesion: 0.25
Nodes (7): 关键决策, 机器人「重启」按钮独立权限点 robot:manage:restart, 涉及范围, 状态, 约束与备注, 记录日期, 需求描述

### Community 1593 - "Community 1593"
Cohesion: 0.29
Nodes (6): 2026-08-27 限流 429 跨域 Network Error 修复, 变更内容, 根因, 约束与备注, 记录日期, 需求背景
...
Cohesion: 0.40
Nodes (5): int, str, clear_login_failure(), record_login_failure(), warmup_blacklist()
...
## Knowledge Gaps
- **3653 isolated node(s):** `allow`, `PreToolUse`, `allow`, `additionalDirectories`, `recommendations` (+3648 more)
...
- **Why does `Client` connect `Community 1451` to `Community 58`, `Community 28`, `Community 863`?**
  _High betweenness centrality (0.063) - this node is a cross-community bridge._
- **Why does `RequestError` connect `Community 58` to `Community 1`, `Community 1442`, `Community 4`, `Community 5`, `Community 41`, `Community 1195`, `Community 1451`, `Community 1200`, `Community 529`, `Community 863`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `NotFoundError` connect `Community 1226` to `Community 0`, `Community 1`, `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 136`, `Community 9`, `Community 15`, `Community 529`, `Community 147`, `Community 21`, `Community 23`, `Community 1438`, `Community 30`, `Community 543`, `Community 1442`, `Community 34`, `Community 1443`, `Community 41`, `Community 1321`, `Community 1195`, `Community 1200`, `Community 51`, `Community 54`, `Community 55`, `Community 58`, `Community 59`, `Community 1344`, `Community 1219`, `Community 76`, `Community 1498`, `Community 609`, `Community 1510`, `Community 1255`, `Community 488`, `Community 1257`, `Community 886`, `Community 631`, `Community 509`, `Community 1022`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Are the 330 inferred relationships involving `NotFoundError` (e.g. with `FastAPI` and `Request`) actually correct?**
  _`NotFoundError` has 330 INFERRED edges - model-reasoned connections that need verification._
- **Are the 315 inferred relationships involving `ResponseModel` (e.g. with `FastAPI` and `Request`) actually correct?**
  _`ResponseModel` has 315 INFERRED edges - model-reasoned connections that need verification._
- **Are the 317 inferred relationships involving `PageRequest` (e.g. with `SysConfigQueryParams` and `PageRequest`) actually correct?**
  _`PageRequest` has 317 INFERRED edges - model-reasoned connections that need verification._
- **Are the 219 inferred relationships involving `BaseRespEntity` (e.g. with `SysConfigBaseResp` and `SysDictSimpleResponse`) actually correct?**
  _`BaseRespEntity` has 219 INFERRED edges - model-reasoned connections that need verification._