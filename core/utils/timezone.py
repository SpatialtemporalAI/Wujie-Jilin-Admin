...
class TimeZone:
    ...
    def from_str(
        self, t_str: str, format_str: str = settings.DATETIME.FORMAT
    ) -> datetime:
        """
        将时间字符串转换为当前时区的 datetime 对象
        :param t_str: 时间字符串
        :param format_str: 时间格式字符串，默认为 settings.DATETIME.FORMAT
        :return:
        """
        return datetime.strptime(t_str, format_str).replace(tzinfo=self.tz_info)
    @staticmethod
    def to_str(t: datetime, format_str: str = settings.DATETIME.FORMAT) -> str:
        """
