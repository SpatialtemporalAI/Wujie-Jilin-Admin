...
from core.exception import errors
from core.models.dataclasses import SnowflakeInfo
...
class Snowflake:
    ...
    def generate(self) -> int:
        ...
        if relative_timestamp >= (1 << SnowflakeConfig.TIMESTAMP_BITS):
            raise errors.ServerError(
                "雪花ID时间戳超出最大范围，需调整EPOCH或增加时间戳位数"
            )
        # 生成雪花ID
        snowflake_id = (
        ...
        if snowflake_id >= (1 << 53):
            raise errors.ServerError(
                f"生成的雪花ID({snowflake_id})超出JavaScript安全整数范围"
            )
        return snowflake_id
    ...
    def parse_id(snowflake_id: int) -> SnowflakeInfo:
        ...
        return SnowflakeInfo(
            ...
            datetime=time.strftime(
                settings.DATETIME.FORMAT, time.localtime(timestamp / 1000)
            ),
