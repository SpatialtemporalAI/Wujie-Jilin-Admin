...
- `app/models/common/base.py` — Pydantic Schema 基类（`BaseEntity`、`BaseReqEntity`、`BaseRespEntity`、`BoolField`）
- `app/models/common/page.py` — 分页基类（`PageRequest`、`get_paginated_results`）
- `app/models/common/mixin.py` — Schema Mixin

## 关键决策

- ORM 层和 Pydantic Schema 层分离：ORM 在 `database/`，Pydantic 在 `app/models/common/`
