...
- **stats 统计**：`func.count/avg` + `group_by`；今日边界用 `database.utils.timezone`（Asia/Shanghai 自然日转 UTC）；总量环比 = 全量 vs 截止上周日累计；今日环比 = 今日 vs 昨日；平均时长 = 当日均值、环比昨日均值（自然日窗口）；**意图分布按轮次表 `VoiceConsultationTurn.intent_type` 统计（join 会话表应用筛选，空意图轮次不计入）**，触发分布仍按会话表；分布 7/2 项 Python 侧补零 + 未知 code 兜底追加。
