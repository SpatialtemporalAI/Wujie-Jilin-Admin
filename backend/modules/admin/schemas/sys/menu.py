...
class SysMenuCreate(BaseEntity):
    ...
    meta_icon_type: str = Field("1", description="图标类型：1-iconify，2-本地")
...
class SysMenuUpdate(BaseEntity):
    ...
    permission: Optional[str] = Field(None, description="权限标识", max_length=100)
    meta_icon: Optional[str] = Field(None, description="路由图标", max_length=50)
    meta_icon_type: Optional[str] = Field(None, description="图标类型：1-iconify，2-本地")
...
class SysMenuResponseData(BaseRespEntity):
    ...
    iconType: str = Field("1", validation_alias=AliasChoices("meta_icon_type", "iconType"), description="图标类型：1-iconify，2-本地")
