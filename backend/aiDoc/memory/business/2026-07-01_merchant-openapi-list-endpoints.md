...
| POST | `/openapi/v1/tasks` | `{robot_sn?,map_id?,task_type?}` | `{tasks: [{id,name,task_type,status,enabled,map_id,last_run_at,next_run_at}]}` |
...
- **任务可见范围 = 按机器人归属**：经 `task_robot` 关联到该商户机器人的任务（与 `execute_task(robot_sn+task_id)` 的 robot 驱动语义一致）；可选 `map_id/task_type` 过滤。
